...
func coderAgentHandler(ctx context.Context, task *a2aSchema.Task, updates chan<- a2a.A2AYieldUpdate) error {
	...
	workingStatus := a2aSchema.TaskStatus{
		...
		Message: &a2aSchema.Message{
			Role:  "agent",
			Parts: []a2aSchema.Part{a2aSchema.Part(`{"type":"text", "text":"Generating code..."}`)},
		},
		Timestamp: time.Now(),
	}
	select {
...
This project was generated by the Go A2A Coder Agent.`,
	...
	for i, filename := range filenames {
		...
		artifact := a2aSchema.Artifact{
			...
			Parts: []a2aSchema.Part{
				a2aSchema.Part(fmt.Sprintf(`{"type":"file", "file": {"name": "%s", "mimeType": "%s", "bytes": "%s"}}`,
					filename, mimeType, fileContentBase64)),
				// Alternative using TextPart if files are small and text-based:
				// a2aSchema.Part(fmt.Sprintf(`{"type":"text", "text":"%s"}`, strings.ReplaceAll(content, `"`, `\"`))),
			},
			Index:     i,                                      // Use index for ordering
			LastChunk: func(b bool) *bool { return &b }(true), // Mark each file artifact as complete
		}
	...
	completedStatus := a2aSchema.TaskStatus{
		...
		Message: &a2aSchema.Message{
			Role:  "agent",
			Parts: []a2aSchema.Part{a2aSchema.Part(`{"type":"text", "text":"` + summary + `"}`)},
		},
		Timestamp: time.Now(),
	}
	select {
