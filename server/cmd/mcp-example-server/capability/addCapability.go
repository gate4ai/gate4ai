package capability

import (
	"encoding/base64"
	"encoding/json"
	"fmt"
	"strconv"
	"time"

	"github.com/gate4ai/mcp/server/mcp/capability"
	"github.com/gate4ai/mcp/shared"
	schema2024 "github.com/gate4ai/mcp/shared/mcp/2024/schema"
	"github.com/gate4ai/mcp/shared/mcp/2025/schema"
)

// Add starts the MCP SSE server with example resources
func Add(
	toolsCapability *capability.ToolsCapability,
	resourcesCapability *capability.ResourcesCapability,
	promptsCapability *capability.PromptsCapability,
	completionCapability *capability.CompletionCapability) {
	// Initialize example tools
	toolsCapability.AddTool(
		"echo",
		"echo a message",
		&schema.JSONSchemaProperty{
			Type: "object",
			Properties: map[string]schema.JSONSchemaProperty{
				"message": {
					Type:        "string",
					Description: "The message to echo back",
				},
			},
			Required: []string{"message"},
		},
		nil,
		func(_ *shared.Message, arguments schema.Arguments) (*schema.Meta, []schema.Content, error) {
			text := "Echo: " + arguments["message"].(string)
			return nil, []schema.Content{{
				Type: "text",
				Text: &text,
			}}, nil
		})

	toolsCapability.AddTool(
		"add",
		"add two numbers",
		&schema2024.JSONSchemaProperty{
			Type: "object",
			Properties: map[string]schema2024.JSONSchemaProperty{
				"a": {
					Type:        "number",
					Description: "First number to add",
				},
				"b": {
					Type:        "number",
					Description: "Second number to add",
				},
			},
			Required: []string{"a", "b"},
		},
		nil,
		func(_ *shared.Message, arguments schema.Arguments) (*schema.Meta, []schema.Content, error) {
			a := arguments["a"].(float64)
			b := arguments["b"].(float64)
			result := strconv.Itoa(int(a + b))
			return nil, []schema.Content{{
				Type: "text",
				Text: &result,
			}}, nil
		})

	toolsCapability.AddTool(
		"longRunningOperation",
		"long running operation",
		&schema2024.JSONSchemaProperty{
			Type: "object",
			Properties: map[string]schema2024.JSONSchemaProperty{
				"duration": {Type: "number"},
				"steps":    {Type: "number"},
			},
			Required: []string{"duration"},
		},
		nil,
		func(_ *shared.Message, arguments schema.Arguments) (*schema.Meta, []schema.Content, error) {
			duration := arguments["duration"].(float64)
			//_ := arguments["steps"].(float64)

			time.Sleep(time.Duration(duration) * time.Second)

			result := "completed"
			return nil, []schema.Content{{
				Type: "text",
				Text: &result,
			}}, nil
		})
	toolsCapability.AddTool(
		"sampleLLM",
		"The prompt to send to the LLM",
		&schema.JSONSchemaProperty{
			Type: "object",
			Properties: map[string]schema.JSONSchemaProperty{
				"prompt":    {Type: "string"},
				"maxTokens": {Type: "number"},
			},
			Required: []string{"prompt", "maxTokens"},
		},
		nil,
		func(msg *shared.Message, arguments schema.Arguments) (*schema.Meta, []schema.Content, error) {
			prompt := arguments["prompt"].(string)
			maxTokens := arguments["maxTokens"].(float64)

			text := "Resource sampleLLM context: " + prompt

			// Get the session from the message
			responce := <-msg.Session.SendRequestSync(
				"sampling/createMessage",
				schema.CreateMessageRequestParams{
					Messages: []schema.SamplingMessage{{
						Role: "user",
						Content: schema.Content{
							Type: "text",
							Text: &text,
						},
					}},
					SystemPrompt:   "You are a helpful test server.",
					MaxTokens:      int(maxTokens),
					Temperature:    &[]float64{0.7}[0],
					IncludeContext: "thisServer",
				},
			)

			if responce.Error != nil {
				return nil, nil, responce.Error
			}

			var createMessageResult schema.CreateMessageResult
			if err := json.Unmarshal(*responce.Result, &createMessageResult); err != nil {
				return nil, nil, err
			}

			resultStr := "LLM sampling result: " + *createMessageResult.Content.Text
			return nil, []schema.Content{{
				Type: "text",
				Text: &resultStr,
			}}, nil
		})
	toolsCapability.AddTool(
		"getTinyImage",
		"Returns the MCP_TINY_IMAGE",
		&schema.JSONSchemaProperty{
			Type:       "object",
			Properties: map[string]schema.JSONSchemaProperty{},
			Required:   []string{},
		},
		nil,
		func(_ *shared.Message, arguments schema.Arguments) (*schema.Meta, []schema.Content, error) {
			text1 := "This is a tiny image:"
			mimeType := "image/png"
			data := ""
			text2 := "The image above is the MCP tiny image."
			return nil, []schema.Content{{
				Type: "text",
				Text: &text1,
			}, {
				Type:     "image",
				Data:     &data,
				MimeType: &mimeType,
			}, {
				Type: "text",
				Text: &text2,
			}}, nil
		})
	toolsCapability.AddTool(
		"printEnv",
		"Returns the printEnv",
		&schema.JSONSchemaProperty{
			Type:       "object",
			Properties: map[string]schema.JSONSchemaProperty{},
			Required:   []string{},
		},
		nil,
		func(_ *shared.Message, arguments schema.Arguments) (*schema.Meta, []schema.Content, error) {
			text := "{\"Fake\": \"envs\"}"
			return nil, []schema.Content{{
				Type: "text",
				Text: &text,
			}}, nil
		})

	// Add prompt handlers with correct signature
	promptsCapability.AddPrompt("simple_prompt", "A simple prompt without arguments",
		func(msg *shared.Message) (*schema.Meta, []schema.PromptMessage, error) {
			responseText := "This is a simple prompt without arguments."
			return nil, []schema.PromptMessage{{
				Role: "user",
				Content: schema.Content{
					Type: "text",
					Text: &responseText,
				},
			}}, nil
		})

	// Complex prompt with required and optional parameters
	promptsCapability.AddPrompt("complex_prompt", "Advanced prompt demonstrating argument handling",
		func(msg *shared.Message) (*schema.Meta, []schema.PromptMessage, error) {
			// Parse parameters from the message
			var params schema.GetPromptRequestParams
			if msg.Params != nil {
				if err := json.Unmarshal(*msg.Params, &params); err != nil {
					return nil, nil, fmt.Errorf("failed to parse parameters: %v", err)
				}
			}

			// Get the temperature parameter (required)
			tempStr, ok := params.Arguments["temperature"]
			if !ok {
				return nil, nil, fmt.Errorf("missing required parameter: temperature")
			}

			// Get the style parameter (optional)
			style, hasStyle := params.Arguments["style"]
			if !hasStyle {
				style = "standard" // Default style
			}

			// Construct the multi-turn conversation with image
			userText := fmt.Sprintf("This is a complex prompt using temperature: %s and style: %s", tempStr, style)
			assistantText := "I'll demonstrate a multi-turn conversation with image response."
			userFollowupText := "Thanks for the detailed response with image!"

			// Create and encode a sample image
			mimeType := "image/png"
			data := ""

			// Create a multi-turn conversation with image
			return nil, []schema.PromptMessage{
				{
					Role: "user",
					Content: schema.Content{
						Type: "text",
						Text: &userText,
					},
				},
				{
					Role: "assistant",
					Content: schema.Content{
						Type: "text",
						Text: &assistantText,
					},
				},
				{
					Role: "assistant",
					Content: schema.Content{
						Type:     "image",
						Data:     &data,
						MimeType: &mimeType,
					},
				},
				{
					Role: "user",
					Content: schema.Content{
						Type: "text",
						Text: &userFollowupText,
					},
				},
			}, nil
		})

	// Initialize example resources
	for i := 1; i <= 100; i++ {
		uri := fmt.Sprintf("test://static/resource/%d", i)
		resourceName := fmt.Sprintf("Resource %d", i)

		if i%2 == 1 {
			// Odd index - plaintext resources
			_ = resourcesCapability.AddResource(
				uri,
				resourceName,
				"A static resource with a numeric ID",
				"text/plain",
				func(msg *shared.Message) (schema.Meta, []schema.ResourceContent, error) {
					uri := fmt.Sprintf("test://static/resource/%d", i)
					text := fmt.Sprintf("Resource %d: This is a plaintext resource", i)
					return nil, []schema.ResourceContent{{
						URI:      uri,
						MimeType: "text/plain",
						Text:     &text,
					}}, nil
				})
		} else {
			// Even index - binary/base64 blob resources
			_ = resourcesCapability.AddResource(
				uri,
				resourceName,
				"A static resource with a numeric ID",
				"application/octet-stream",
				func(msg *shared.Message) (schema.Meta, []schema.ResourceContent, error) {
					uri := fmt.Sprintf("test://static/resource/%d", i)
					data := fmt.Sprintf("Resource %d: This is a base64 blob", i)
					encodedData := base64.StdEncoding.EncodeToString([]byte(data))
					return nil, []schema.ResourceContent{{
						URI:      uri,
						MimeType: "application/octet-stream",
						Blob:     &encodedData,
					}}, nil
				})
		}
	}

	go func() {
		for {
			time.Sleep(5 * time.Second)
			for _, uri := range resourcesCapability.GetSubscribedResources() {
				resourcesCapability.NotifyResourceUpdated(uri)
			}
		}
	}()

	// Add template for resource URIs
	resourcesCapability.AddResourceTemplate(
		"test://static/resource/{id}",
		"Static Resource",
		"A static resource with a numeric ID",
		"",
		nil,
	)
}
