    ...
    <v-col cols="12" sm="8" md="6" lg="4">
      <v-card class="pa-4">
          ...
          <v-form ref="form" @submit.prevent="requestPasswordReset">
            <v-card-text>
              ...
              <div v-if="requestSent" class="text-center">
                ...
                <v-btn variant="text" color="primary" to="/login" class="mt-4">
                  Back to Login
                </v-btn>
              </div>
              ...
              <div v-if="!requestSent" class="text-center mt-2">
                <v-btn variant="text" color="grey-darken-1" to="/login">
                  Cancel
                </v-btn>
              </div>
            </v-card-text>
        ...
        <div v-else class="pa-4 text-center">
          <v-icon color="warning" size="x-large" class="mb-4"
          ...
          <p class="text-body-2">
            Please contact the administrators for assistance. If you are an
            admin, check the 'Email' settings.
          </p>
          <v-btn color="primary" to="/login" class="mt-6">
            Back to Login
          </v-btn>
        </div>
      </v-card>
...
onMounted(() => {
  ...
  if (!$settings.isLoaded()) {
    console.warn(
      "Settings not loaded on forgot password page mount. Email disabled message might be shown initially."
    );
