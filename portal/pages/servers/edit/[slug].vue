  ...
  <div>
    <v-container>
      ...
      <!-- NEW: Headers Management Section -->
       <v-card v-if="serverDataForForm && canManageServer" class="pa-4 mb-6">
         <v-card-title class="text-h5 mb-3">Server Headers</v-card-title>
          <p class="text-caption mb-4">Headers automatically sent by the gateway to this server.</p>
      ...
      <!-- NEW: Subscription Header Template Management Section -->
       <v-card v-if="serverDataForForm && canManageServer" class="pa-4 mb-6">
         <v-card-title class="text-h5 mb-3">Subscription Header Template</v-card-title>
          <p class="text-caption mb-4">Define headers subscribers must provide.</p>
...
interface HeaderRecord { [key: string]: string }
...
async function fetchServer() {
  ...
  try {
    ...
    const data = await $api.getJson<ServerApiResponse & { owners: { user: { id: string }}[] }>(`/servers/${serverSlug}`);
...
async function fetchServerHeaders() {
    try {
        currentServerHeaders.value = await $api.getJson<HeaderRecord>(`/servers/${serverSlug}/headers`);
    } catch (err) {
...
async function fetchSubscriptionTemplate() {
     try {
        currentSubscriptionTemplate.value = await $api.getJson<TemplateItem[]>(`/servers/${serverSlug}/subscription-header-template`);
    } catch (err) {
