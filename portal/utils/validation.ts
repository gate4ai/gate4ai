...
export const rules = {
   ...
   * @returns True if valid or error message
   */
  email: (v: string): boolean | string => !v || /.+@.+\..+/.test(v) || 'Please enter a valid email',
   ...
   * @returns True if valid or error message
   */
  password: (v: string): boolean | string => (v && v.length >= 8) || 'Password must be at least 8 characters',
   ...
   */
  url: (v: string): boolean | string => !v || /^https?:\/\/[^\s$.?#].[^\s]*$/.test(v) || 'Please enter a valid URL',
   ...
   * @returns True if valid, true if empty, or error message
   */
  simpleUrl: (v: string): boolean | string => !v || /^https?:\/\//.test(v) || 'URL must start with http:// or https://',
  ...
  serverUrl: [
    ...
    (v: string): boolean | string => /^https?:\/\//.test(v) || 'URL must start with http:// or https://'
  ],
   ...
   */
  confirmPassword: (password: string) => (v: string): boolean | string =>
   ...
   */
  agree: (v: boolean): boolean | string => v || 'You must agree to the terms to continue',
  ...
  slugFormat: (v: string): boolean | string =>
    /^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(v) || 'Slug must contain only lowercase letters, numbers, and hyphens, and cannot start or end with a hyphen.',
