...
export default defineNuxtPlugin({
  ...
  async setup(_nuxtApp) { // Renamed to _nuxtApp as we use useNuxtApp() inside loadSettings
    ...
    async function loadSettings() {
      ...
      try {
        ...
        console.info('[Plugin 03.settings.ts] Attempting to load frontend settings via $api.getJson...');
        ...
        console.info(`[Plugin 03.settings.ts] Frontend settings data received: ${data ? `(${data.length} items)` : 'null/undefined'}`);
        ...
        if (data && Array.isArray(data)) {
          ...
          console.warn('[Plugin 03.settings.ts] Received non-array or empty data for frontend settings.');
        }
        ...
        const errorMsg = error instanceof Error ? error.message : 'Failed to load settings due to an unknown error';
        console.error('[Plugin 03.settings.ts] Error during loadSettings execution:', errorMsg, error);
    ...
    if (import.meta.server) {
        _nuxtApp.hooks.hookOnce('app:created', async () => {
           console.log('[Plugin 03.settings.ts] [SSR] app:created hook triggered.');
           if (!state.value.loaded) { // Check just in case (shouldn't be loaded yet on SSR)
               console.log('[Plugin 03.settings.ts] [SSR] Attempting loadSettings from app:created.');
               await loadSettings(); // Await the loading process
           } else {
                console.log('[Plugin 03.settings.ts] [SSR] Settings already marked as loaded in app:created, skipping load (unexpected).');
           }
        });
    }
    ...
    if (import.meta.client) {
      _nuxtApp.hooks.hookOnce('app:mounted', async () => {
         console.log('[Plugin 03.settings.ts] [Client] app:mounted hook triggered.');
         // Only execute load if the state wasn't successfully loaded/hydrated from the server
         if (!state.value.loaded) {
             console.log('[Plugin 03.settings.ts] [Client] Settings not loaded, executing loadSettings from app:mounted hook.');
             await loadSettings();
         } else {
             console.log('[Plugin 03.settings.ts] [Client] Settings already loaded (likely hydrated from server), skipping load in app:mounted.');
         }
      });
    }
    ...
    return {
      provide: {
        settings: {
          ...
          getAll: (): Readonly<Record<string, unknown>> => Object.freeze({ ...state.value.settings }), // Return immutable copy
          ...
          reload: async () => {
            console.info('[Plugin 03.settings.ts] Reload requested. Forcing settings reload...');
