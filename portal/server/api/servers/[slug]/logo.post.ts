...
const ALLOWED_MIME_TYPES = ['image/png', 'image/jpeg', 'image/svg+xml', 'image/gif'];
...
export default defineEventHandler(async (event) => {
    ...
    try {
        ...
        console.log(`[${slug}/logo.post] Received formData parts:`, formData?.map(p => ({ name: p.name, filename: p.filename, type: p.type, size: p.data?.length })));
        ...
        console.log(`[${slug}/logo.post] Found 'logoFile' part details:`, logoFile ? { name: logoFile.name, filename: logoFile.filename, type: logoFile.type, hasData: !!logoFile.data, dataLength: logoFile.data?.length } : 'Part Not Found');

        // 3. Validate the found file part more carefully
        if (!logoFile) {
            throw createError({ statusCode: 400, statusMessage: "Missing 'logoFile' part in the form data." });
        }
        // Check essential properties *after* confirming logoFile exists
        if (!logoFile.filename) {
            throw createError({ statusCode: 400, statusMessage: "Uploaded file is missing a filename." });
        }
        if (!logoFile.type) {
             throw createError({ statusCode: 400, statusMessage: "Uploaded file is missing a content type." });
        }
        // Check if data exists and has length > 0
        if (!logoFile.data || logoFile.data.length === 0) {
            throw createError({ statusCode: 400, statusMessage: "Uploaded file data is empty." });
        }

        // 4. Validate File Content Type and Size
        if (!ALLOWED_MIME_TYPES.includes(logoFile.type)) {
            throw createError({ statusCode: 400, statusMessage: `Invalid file type '${logoFile.type}'. Allowed types: ${Object.values(mimeToExt).join(', ')}.` });
        }
        if (logoFile.data.length > MAX_FILE_SIZE_BYTES) {
            throw createError({ statusCode: 400, statusMessage: `File is too large (${(logoFile.data.length / 1024 / 1024).toFixed(2)}MB). Maximum size is ${MAX_FILE_SIZE_MB}MB.` });
        }
        ...
        if (!extension) {
            ...
            throw createError({ statusCode: 500, statusMessage: 'Internal server error: Could not determine file extension from allowed type.' });
        }
        ...
        try {
            ...
            console.log(`[${slug}/logo.post] Logo saved to ${filePath}`);
             ...
             console.error(`[${slug}/logo.post] Error writing file to ${filePath}:`, writeError);
        ...
        try {
            ...
            console.log(`[${slug}/logo.post] Database updated with imageUrl: ${imageUrl}`);
        } catch (dbError) {
             console.error(`[${slug}/logo.post] Error updating database for server ${server.id}:`, dbError);
             // Attempt to clean up saved file if DB update fails? Optional.
             try { await fs.unlink(filePath); } catch (cleanupErr) { console.error(`[${slug}/logo.post] Failed to clean up file ${filePath} after DB error:`, cleanupErr); }
             throw createError({ statusCode: 500, statusMessage: 'Failed to update server record with new logo URL.' });
        }
        ...
        console.error(`[${slug}/logo.post] Error during logo upload process:`, error);
        ...
        throw createError({ statusCode: 500, statusMessage: 'Failed to upload logo due to an unexpected server error.' });
    }
