...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const subscription = await prisma.subscription.findUnique({
      ...
      include: {
        server: { // Include server and its owners for permission check
          select: { id: true, owners: { select: { id: true } } }
        }
      }
    });

    if (!subscription || !subscription.server) { // Check server relation exists
      throw createError({ statusCode: 404, statusMessage: 'Subscription or associated server not found' });
    }

    // 3. Permission Check: Only Admin/Security or Server Owner can change status
    const { isOwner, isAdminOrSecurity } = getServerReadAccessLevel(user, subscription.server);
    if (!isAdminOrSecurity && !isOwner) {
      throw createError({ statusCode: 403, statusMessage: 'Forbidden: You do not have permission to change subscription status.' });
    }
    ...
    throw createError({ statusCode: 500, statusMessage: 'Failed to update subscription status' });
  }
});