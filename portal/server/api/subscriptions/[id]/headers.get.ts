...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const { subscription } = await checkSubscriptionAccessRights(
      event,
      subscriptionId
    );
    ...
    console.error(
      `Error fetching subscription headers for ID ${subscriptionId}:`,
      error
    );
    ...
    throw createError({
      statusCode: 500,
      statusMessage: "Failed to fetch subscription headers",
    });
  }
});
