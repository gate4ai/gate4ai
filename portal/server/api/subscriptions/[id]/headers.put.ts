  ...
  try {
    ...
    if (!isSubscriber) {
      throw createError({
        statusCode: 403,
        statusMessage:
          "Forbidden: Only the subscriber can update these headers.",
      });
    }
    ...
    if (!validationResult.success) {
      throw createError({
        statusCode: 400,
        statusMessage: "Validation Error: Invalid header values format.",
    ...
    if (Object.keys(validationErrors).length > 0) {
      throw createError({
        statusCode: 400,
        statusMessage:
          "Validation Error: Header values do not match template requirements.",
        data: validationErrors,
      });
    }
    const updatedSubscription = await prisma.subscription.update({
      where: { id: subscriptionId },
      data: { headerValues: newHeaderValues as Prisma.JsonObject },
      select: { headerValues: true },
    });
    return updatedSubscription.headerValues ?? {};
  } catch (error: unknown) {
    console.error(
      `Error updating subscription headers for ID ${subscriptionId}:`,
      error
    );
    if (
    ...
    throw createError({
      statusCode: 500,
      statusMessage: "Failed to update subscription headers",
    });
  }
});
