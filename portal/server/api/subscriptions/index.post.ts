...
const headerValuesSchema = z.record(z.string(), z.string(), {
    invalid_type_error: "Header values must be strings.",
}).optional().nullable(); // Optional map[string]string
  ...
  try {
    ...
    for (const providedKey in headerValues) {
        if (!templateKeys.has(providedKey)) {
            validationErrors[providedKey] = [`Header '${providedKey}' is not defined for this server.`];
        }
    }
    ...
    if (Object.keys(validationErrors).length > 0) {
        throw createError({
            statusCode: 400,
            statusMessage: 'Validation Error: Provided headers do not meet requirements.',
    ...
    const newSubscription = await prisma.subscription.create({
      data: {
        ...
        headerValues: Object.keys(validatedHeaderValues).length > 0 ? validatedHeaderValues as Prisma.JsonObject : Prisma.JsonNull, // Save validated headers or null
      },
