...
async function getEmailSettings(): Promise<{ smtpConfig: SmtpConfig | null, doNotSend: boolean }> {
    const now = new Date();
    // Check cache validity
    if (lastSettingsCheck !== null && (now.getTime() - lastSettingsCheck.getTime()) < CACHE_DURATION) {
   ...
   try {
        ...
        settings.forEach(setting => {
            if (setting.key === 'email_do_not_send_email' && typeof setting.value === 'boolean') {
                 ...
                 if (typeof val === 'object' && val !== null && typeof val.host === 'string' && typeof val.port === 'number') {
                     ...
                     console.error("Invalid format for email_smtp_server setting:", setting.value);
                 }
            }
        });
...
export async function sendEmail(to: string, subject: string, htmlBody: string): Promise<void> {
    ...
    if (doNotSend) {
        console.log(`Email sending skipped (disabled by setting) for subject: ${subject}`);
        return; // Don't send if disabled
    }

    if (!smtpConfig || !smtpConfig.host) {
        console.error('SMTP server configuration is missing or invalid. Cannot send email.');
        throw createError({ statusCode: 500, statusMessage: 'Email configuration error' });
    }
