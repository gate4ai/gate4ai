<template>
  <v-dialog :model-value="modelValue" max-width="500px" persistent @update:model-value="closeDialog">
    <v-card>
...
const fileRules = computed(() => [
  (file: File | null): boolean | string => { // Rule now accepts File | null
      if (!file) return true; // No file selected is valid in terms of rules
      return file.size <= MAX_FILE_SIZE_BYTES || `File size should be less than ${MAX_FILE_SIZE_MB} MB`;
  }
]);
