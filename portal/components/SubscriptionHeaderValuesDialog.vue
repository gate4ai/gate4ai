<template>
    <v-dialog :model-value="modelValue" max-width="600px" persistent scrollable @update:model-value="closeDialog">
      <v-card>
        <v-card-title>{{ editMode ? 'Edit' : 'Configure' }} Subscription Headers</v-card-title>
  
        <v-form ref="formRef" @submit.prevent="saveValues">
          <v-card-text>
            <p class="text-caption mb-4">
              Please provide values for the headers required by this server. These values will be sent with your requests.
            </p>
  
            <div v-if="isLoadingTemplate || isLoadingValues" class="text-center pa-4">
          ...
          <v-card-actions>
            <v-spacer />
            <v-btn color="grey-darken-1" variant="text" :disabled="isSaving || isLoadingTemplate || isLoadingValues" @click="closeDialog">
              Cancel
            </v-btn>
            <v-btn color="primary" type="submit" :loading="isSaving" :disabled="isLoadingTemplate || isLoadingValues">
  ...
  async function fetchTemplate() {
    ...
    try {
      headerTemplate.value = await $api.getJson<TemplateItem[]>(`/servers/${props.serverSlug}/subscription-header-template`);
      ...
      loadError.value = err instanceof Error ? err.message : 'Failed to load header template.';
  ...
  async function fetchExistingValues() {
    ...
    try {
      const existingValues = await $api.getJson<Record<string, string>>(`/subscriptions/${props.subscriptionId}/headers`);
      // Merge existing values with defaults from template
       ...
       loadError.value = err instanceof Error ? err.message : 'Failed to load existing header values.';
       showError(loadError.value);
    } finally {
  ...
  async function saveValues() {
    ...
    try {
      ...
      if (editMode.value && props.subscriptionId) {
        ...
        await $api.putJson(`/subscriptions/${props.subscriptionId}/headers`, valuesToSave);
      ...
      const message = err instanceof Error ? err.message : 'Failed to save header values.';
