...
async function handleClick() {
  ...
  if (!props.isAuthenticated) {
    router.push(`/login?redirect=/servers/${props.server.slug}`);
    return;
  }
...
async function performUnsubscribe() {
  ...
  try {
    ...
    await $api.deleteJson(
      `/subscriptions/${localSubscriptionState.subscriptionId}`
    );
  ...
  try {
    if (headerTemplateCache.value === null) {
      headerTemplateCache.value = await $api.getJson<TemplateItem[]>(
        `/servers/${props.server.slug}/subscription-header-template`
      );
    }
    if (headerTemplateCache.value?.length > 0) {
    ...
    const message =
      err instanceof Error ? err.message : "Failed to check requirements.";
    showError(message);
    console.error("Error checking template:", err);
  } finally {
    if (!showHeadersDialog.value) isLoading.value = false;
  }
}
async function handleHeadersSave(headerValues: Record<string, string>) {
  /* ... */
  showHeadersDialog.value = false;
  await performSubscribe(headerValues);
}
async function performSubscribe(headerValues?: Record<string, string>) {
  /* ... */
  isLoading.value = true;
  try {
    const payload: { serverId: string; headerValues?: Record<string, string> } =
      { serverId: props.server.id };
    if (headerValues) payload.headerValues = headerValues;
    const newSubscription = await $api.postJson<SubscriptionResponse>(
      "/subscriptions",
      payload
    );
    localSubscriptionState.isSubscribed = true;
    localSubscriptionState.subscriptionId = newSubscription.id;
...
watch(
  () => [props.server.isCurrentUserSubscribed, props.server.subscriptionId],
