      ...
      >
        <v-expansion-panel-title>
          <div class="d-flex flex-column align-start">
            ...
            <span v-if="skill.description" class="text-caption text-grey">{{ skill.description }}</span>
          </div>
        </v-expansion-panel-title>
        <v-expansion-panel-text>
          <div v-if="skill.tags && skill.tags.length > 0" class="mb-4">
            ...
            <v-chip-group>
              <v-chip v-for="tag in skill.tags" :key="tag" color="info" size="small">
                {{ tag }}
              </v-chip>
            </v-chip-group>
          </div>
          ...
          <div v-if="skill.examples && skill.examples.length > 0" class="mb-4">
            ...
            <v-list>
              <v-list-item v-for="(example, index) in skill.examples" :key="index">
                <v-list-item-title>
          ...
          <div v-if="skill.inputModes && skill.inputModes.length > 0" class="mb-2">
            <h3 class="text-subtitle-1 mb-1">Input Modes</h3>
            <v-chip-group>
              <v-chip v-for="mode in skill.inputModes" :key="mode" color="primary" size="small">
                {{ mode }}
              </v-chip>
            </v-chip-group>
          </div>
          ...
          <div v-if="skill.outputModes && skill.outputModes.length > 0">
            ...
            <v-chip-group>
              <v-chip v-for="mode in skill.outputModes" :key="mode" color="success" size="small">
                {{ mode }}
              </v-chip>
            </v-chip-group>
          </div>
