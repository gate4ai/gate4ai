<template>
  <div>
    ...
    <v-progress-linear v-if="isListLoading" indeterminate color="primary" class="mb-2" />
    ...
    <v-list v-if="!isListLoading && localOwners.length > 0">
     ...
     <v-list-item v-else-if="!isListLoading">
       <v-list-item-title class="text-grey">No owners assigned.</v-list-item-title>
     </v-list-item>
    ...
    <v-dialog v-model="removeDialog" max-width="500px">
      <v-card>
        ...
        <v-card-text>
          Are you sure you want to remove <strong>{{ ownerToRemove?.user.name || ownerToRemove?.user.email }}</strong> as an owner?
        </v-card-text>
        <v-card-actions>
          <v-spacer />
          <v-btn color="grey-darken-1" variant="text" @click="removeDialog = false">Cancel</v-btn>
          <v-btn color="error" variant="text" :loading="isActionLoading" @click="removeOwner">Yes, Remove</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
async function addOwner() {
  ...
  try {
    ...
    const updatedOwners = await $api.postJson(`/servers/${props.serverId}/owners`, {
...
async function removeOwner() {
    ...
    try {
        ...
        const updatedOwners = await $api.deleteJson(`/servers/${props.serverId}/owners/${ownerId}`) as ServerOwner[];
