    ...
    <v-card>
      ...
      <v-card-text>
        <p class="text-caption mb-4">
          These headers will be automatically added by the gateway to requests
          sent to this server's URL (<code>{{ serverUrl }}</code
          >). Subscription headers will override these if keys conflict. System
          headers (e.g., `Gate4ai-User-Id`) have the highest priority.
        </p>
...
async function saveHeaders() {
  ...
  try {
    ...
    const updated = await $api.putJson<Record<string, string>>(
      `/servers/${props.serverSlug}/headers`,
      headersToSend
    );
    ...
    const message =
      err instanceof Error ? err.message : "Failed to update server headers.";
    error.value = message;
    showError(message);
  } finally {
